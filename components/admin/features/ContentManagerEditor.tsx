...
export function ContentManagerEditor({
  initialSections,
}: {
  ...
  const [activeSection, setActiveSection] = useState(
    initialSections[0]?.section_name || "",
  );
  ...
  const currentContent =
    sections.find((s) => s.section_name === activeSection)?.content || {};
      ...
      <div className="lg:col-span-9">
        <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col group min-h-[600px]">
          <div className="p-10 flex items-center justify-between border-b border-gray-50">
            <div>
              <h3 className="text-2xl font-black text-gray-900 leading-none capitalize">
                {activeSection} Configuration
              </h3>
              <p className="text-xs text-gray-400 mt-3 font-bold uppercase tracking-widest">
                Update live site contents for this section
              </p>
            </div>
            ...
            {activeSection === "hero" && (
              <div className="space-y-8 animate-in fade-in duration-500">
                <div className="space-y-3 group/field">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1 group-focus-within/field:text-[#9b1d20] transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleUpdateField("code", e.target.value)
                      }
                      className="block w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-[1.25rem] text-sm font-bold focus:outline-none"
                    />
                  </div>
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">
                    Intro Video URL (Direct MP4)
                  </label>
                  <input
                    type="text"
                    value={currentContent.video_url || ""}
                    onChange={(e) =>
                      handleUpdateField("video_url", e.target.value)
                    }
                    className="block w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-[1.25rem] text-sm font-bold focus:outline-none"
                  />
                </div>
            ...
            {activeSection === "fees" && (
              <div className="space-y-8 animate-in fade-in duration-500">
                ...
                {currentContent.rates?.map((rate: any, idx: number) => (
                  <div
                    key={idx}
                    className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-6 bg-gray-50 rounded-3xl border border-gray-100"
                  >
                    <div className="space-y-1">
                      <p className="text-[10px] font-black text-[#9b1d20]/50 uppercase tracking-tighter">
